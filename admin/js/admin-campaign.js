jQuery(document).ready(function($) {
    ...
    function toggleTypeOptions() {
        ...
        // --- UPDATED: Show/Hide the Placements meta box ---
        const $placementsBox = $('#adcp_placements_meta_box');

        if (type === 'embed') {
            // SHOW embed-specific fields (Shortcode)
            $embedFields.show();
            $placementsBox.show(); // <-- NEW
            // Enable its inputs
    ...
    // We use a delegated selector to ensure it works.
    $(document).on('change', '#adcp_placement_select_all', function() {
    ...
    $('#adcp_preview_ad').on('click', function() {
        ...
        const c = {
            ...
            priority: $('#adcp_priority').val(), // <-- NEW: Get z-index for preview
    ...
    function buildElementHTML(c) {
        ...
        // NEW: Add inline style for z-index
        const elStyle = c.priority ? `style="z-index: ${c.priority}"` : '';
        ...
        return `<div class="${elClass}" data-campaign-id="${c.id}" ${elStyle}>${innerHTML}</div>`;
    }
