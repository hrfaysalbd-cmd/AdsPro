...
class Adcp_Public {
    ...
    public function render_campaign_shortcode( $atts ) {
        ...
        if ( $creative_source === 'html' && !empty($config['creative_html']) ) {
            // This is where the Embed Shortcode *should* work.
    ...
    public static function display_ad_for_placement( $placement_key ) {
        ...
        if ( $campaign_id ) {
            ...
            $ad_public_instance = new Adcp_Public( ADCP_VERSION );
            echo $ad_public_instance->render_campaign_shortcode( array( 'id' => $campaign_id ) );
        }
    }
