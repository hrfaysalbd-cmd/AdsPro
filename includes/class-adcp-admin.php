...
class Adcp_Admin {
	...
	public function add_plugin_admin_menu() {
        ...
        add_action( 'load-' . $campaign_edit_hook, array( $this, 'add_placements_meta_box_hook' ) );
     ...
     */
    public function add_placements_meta_box_hook() {
        // This registers the meta box for our custom page screen
        add_meta_box(
            'adcp_placements_meta_box',       // ID
            'Ad Placements (for Embed Ads)',  // Title
            array( $this, 'render_placements_meta_box' ), // Callback
            'campaign',                       // The custom screen ID we use in do_meta_boxes()
            'side',                           // Context
            'default'                         // Priority
        );
    }
     ...
     */
    public function render_placements_meta_box( $post_or_campaign_obj ) {
        // $post_or_campaign_obj is the $campaign object from display_campaign_edit_page
        $campaign_id = 0;
        $config = array();

        // Check if it's a real WP_Post or our dummy object
        if ( $post_or_campaign_obj instanceof WP_Post ) {
            $campaign_id = $post_or_campaign_obj->ID;
        } elseif ( is_object( $post_or_campaign_obj ) && isset( $post_or_campaign_obj->ID ) ) {
            $campaign_id = $post_or_campaign_obj->ID;
        }
        ...
        $all_placements = [
            ...
            'before_footer' => 'Before Footer <small>Before the <code>&lt;footer&gt;</code> tag.</small>',
            'after_footer' => 'After Footer <small>Right before <code>&lt;/body&gt;</code>.</small>',
        ...
        echo '<p style="border-bottom: 1px solid #eee; padding-bottom: 5px;">Select locations to automatically inject this ad. <strong>Only works for "Embed" type ads.</strong></p>';
	...
	public function display_campaign_edit_page() {
        ...
        if ( $is_editing ) {
            ...
            if ( $campaign ) {
             ...
             // Create a dummy object for do_meta_boxes()
                                        ...
                                        <table class="form-table">
                                            <tr valign="top">
                                                ...
                                                <td>
                                                    ...
                                                    <p class="description">HTML will be sanitized on save, but shortcodes [like_this] will be preserved.</p>
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                                    ...
                                    <table class="form-table">
                                        <tr valign="top" class="adcp-type-overlay-show">
                                            ...
                                            <td>
                                                ...
                                                <p class="description">
                                                    One path pattern per line. Use * as wildcard. e.g., <code>/blog/*</code> or <code>/shop/product-name</code>.
                                                    <strong>DO NOT</strong> include the domain (https://...).
                                ...
                                <div class="inside">
                                    ...
                                    <p>
                                        ...
                                        <select id="adcp_type" name="type">
                                            <option value="popup" <?php selected( $get_val('type', 'popup'), 'popup' ); ?>>Popup (Overlay)</option>
                                            <option value="slide" <?php selected( $get_val('type'), 'slide' ); ?>>Slide-in (Overlay)</option>
                                            <option value="scroll" <?php selected( $get_val('type'), 'scroll' ); ?>>Scroll Bar (Overlay)</option>
                                            <option value="embed" <?php selected( $get_val('type'), 'embed' ); ?>>Embed (Inner Content)</option>
                                        </select>
                                    </p>
                                    <p>
                                        ...
                                        <p class="description">
                                            ...
                                            <b>10-100</b> (normal, below headers),
                                            <b>99999+</b> (covers everything).
                                            <b>Your header is 10, so use a value *less than 10* (e.g., 9) for a top bar *behind* it, or 100+ for an ad *over* it.</b>
                                        </p>
                                    </p>
    ...
    public function handle_save_campaign() {
        ...
        // --- FIX: Only check for the placements nonce if the type is 'embed' ---
        if ( isset( $_POST['type'] ) && $_POST['type'] === 'embed' ) {
            if ( ! isset( $_POST['adcp_placements_nonce'] ) || ! wp_verify_nonce( $_POST['adcp_placements_nonce'], 'adcp_save_placements_nonce' ) ) {
                wp_die('Security check failed (placements).');
            }
        }
        ...
        if ( isset( $_POST['config'] ) && is_array( $_POST['config'] ) ) {
            ...
            // --- UPDATED SAVE LOGIC ---
            if ( $data['type'] === 'embed' ) {
                // It's an Embed ad, save placements and clear overlay fields
                ...
                $config_data['device_desktop'] = 0;
                $config_data['device_mobile']  = 0;
                $config_data['device_tablet']  = 0;
                $config_data['freq_cap'] = 0;
                $config_data['target_urls'] = '';

            } else {
                // It's an Overlay ad, save overlay fields and clear placements
                ...
                $config_data['target_urls'] = isset( $config_in['target_urls'] ) ? sanitize_textarea_field( $config_in['target_urls'] ) : '';
            }
    ...
    public function display_guide_page() {
        ...
        <div class="wrap">
            ...
            <p>This guide explains how to show ads site-wide, how to fix the header (z-index) issue, and how to use the "Automatic Placements" for inner content.</p>
            <hr>

            <div class="postbox">
                <h2 class="hndle"><span>Important: Fixing Ads Covering Your Header (z-index)</span></h2>
                <div class="inside">
                    <p>Based on your screenshot, an ad is covering your header. This is because the ad's <b>Priority (z-index)</b> is too high.</p>
                    <p>Your header has a z-index of <b>10</b>. The ad is likely set to a high number like <b>99998</b>.</p>
                    <ol>
                        <li>Go to <a href="<?php echo admin_url('admin.php?page=adcp-campaigns'); ?>">All Campaigns</a> and edit the ad that is covering your header (the "Sign Contract" ad).</li>
                        <li>Find the <b>"Priority (z-index)"</b> field in the sidebar.</li>
                        <li>Change the value to be <b>LESS</b> than your header. Try <b>9</b>.</li>
                        <li>Save the campaign. This will place the ad *behind* your header, which is correct for a top bar.</li>
                    </ol>
                    <p><b>Rule of Thumb:</b></p>
                    <ul>
                        <li><b>Overlays (Popup, Slide-in):</b> Use a HIGH z-index (like <b>99999</b>) to appear on top of everything.</li>
                        <li><b>Top Bars (Scroll type):</b> Use a LOW z-index (like <b>9</b>) to appear *behind* your sticky header.</li>
                    </ul>
                </div>
            </div>
            
            <div class="postbox">
                <h2 class="hndle"><span>How to Show Ads Inside Your Content (Automatic Placements)</span></h2>
                <div class="inside">
                    <p>This is the main solution for your "Inner Content" ads. This method lets you "tag" a campaign to show in a general location (like "Blog Post Sidebar"). The plugin will automatically find an active ad for that location and display it. This is how you get random ads in the same spot.</p>
                    ...
                    <ol>
                        ...
                        <li>Set the campaign <b>Type</b> to <b>"Embed (Inner Content)"</b>.</li>
                    ...
                    <p>You only need to do this once for each location. Add the correct PHP snippet to your theme's files (e.g., <code>header.php</code>, <code>footer.php</code>, <code>sidebar.php</code>).</p>
                    ...
                    <style>
                        ...
                        .adcp-guide-table pre { background: #333; color: #fff; padding: 10px; border-radius: 4px; font-size: 14px; white-space: pre-wrap; }
                    </style>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><pre>&lt;?php do_action('adcp_placement_before_footer'); ?&gt;</pre>
                                    <small>Place this right before your main <code>&lt;footer&gt;</code> element.</small>
                                </td>
                            </tr>
                            <tr>
                                <td><b>After Footer</b><br>(<code>after_footer</code>)</td>
                                <td><code>footer.php</code></td>
                                <td><pre>&lt;?php do_action('adcp_placement_after_footer'); ?&gt;</pre>
                            ...
                            <tr>
                                ...
                                <td><pre>&lt;?php do_action('adcp_placement_blog_sidebar'); ?&gt;</pre>
                                    <small>Place this inside your sidebar container.</small>
                                </td>
                            </tr>
                            <tr>
    ...
    if ( is_single() && ! is_admin() ) {
        ...
        if ( ! empty( $ad_markup ) ) {
            ...
            if ( count( $paragraphs ) > $insert_after ) {
                array_splice( $paragraphs, $insert_after, 0, $ad_markup );
...
if ( class_exists('Adcp_Public') ) {
    ...
    add_action( 'adcp_placement_after_footer', function() {
        Adcp_Public::display_ad_for_placement('after_footer');
    } );
...
// --- End AdsCampaignPro Hooks ---
                    ...
                    <p>By adding these, you can now simply check a box in the "Ad Placements" meta box, and a random, eligible ad will automatically appear in the location you defined.</p>
                </div>
            </div>

            <div class="postbox">
                <h2 class="hndle"><span>Method 2: Manual Placement (Shortcode)</span></h2>
                <div class="inside">
                    <p>This is the simplest way to place an ad in a single, specific location.</p>
                    <ol>
                        <li>Go to <a href="<?php echo admin_url('admin.php?page=adcp-campaigns'); ?>">All Campaigns</a> and create or edit a campaign.</li>
                        <li>Set the campaign <b>Type</b> to <b>"Embed (Inner Content)"</b>.</li>
