 ...
 * v1.1.2 - Fix for z-index and multiple overlays
 */
(function() {
    'use strict';
    ...
    function buildElementFromConfig(c) {
        ...
        // --- Z-INDEX FIX (Request 1) ---
        // The priority (z-index) is now passed from the API.
        // We set it here as an inline style.
        if (c.priority) {
            el.style.zIndex = c.priority;
        }
        ...
        el.querySelector('.adcp-close').addEventListener('click', () => { 
            // We removed the 'paddingTop' pushdown logic as requested,
            // as z-index is the correct fix for your use case.
            el.remove(); 
        });
        return el;
    }
        ...
        fetch(`${API_BASE}/campaigns?url=${pageUrl}&device=${device}&token=${CLIENT_TOKEN || ''}`)
            ...
            .then(data => {
                if (data.campaigns && data.campaigns.length > 0) {
                    ...
                    // --- MULTIPLE ADS FIX (Request 2) ---
                    // This object tracks if we've already rendered one of each type.
                    const renderedTypes = {};
                    
                    data.campaigns.forEach(c => {
                        // Check if we have *not* rendered this type yet
                        // The API already sorts by priority, so this renders
                        // the highest-priority ad for each type.
                        if (!renderedTypes[c.type]) {
                            renderCampaign(c);
                            // Mark this type as rendered
                            renderedTypes[c.type] = true;
                        }
                    });
                    // --- END FIX ---
                }
